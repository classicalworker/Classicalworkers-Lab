...
## Discordの日報(CP増減)の取り込みについて

コミュニティで使っているDiscord Bot「sf6bot_test」は、MR獲得量に応じたCP(コミュニティ内通貨)を
1日ごとの「日報」としてチャンネルに投稿しています。このBotの管理者権限が無くても、
DiscordChatExporter等でチャンネルをJSON形式にエクスポートしてもらえれば、そのファイルを管理者ページから
アップロードするだけでサイトに反映できます(自動更新はできないため、エクスポートするたびに読み込み直してください)。

CPランキングは、取り込んだ「日報」のCP増減を日付ごとに積み上げて算出しています。同じ日付を
読み込み直しても二重計上はされないため、期間が重なった状態で何度エクスポート・取り込みをしても安全です。
...
取り込んだ日々のCP/MR増減は `discordDailyReports` に日付ごとに保存され、ランキングはその合計値を
表示に使用しています。
