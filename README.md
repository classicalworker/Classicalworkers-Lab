...
## Discordログ(CP・tier・日報)の取り込みについて

コミュニティで使っているDiscord Bot「sf6bot_test」は、MR獲得量に応じたCP(コミュニティ内通貨)やtier(名前の色)を管理しており、
「日報」「全メンバー一覧」「個人の総取得MR」をチャンネルに投稿しています。このBotの管理者権限が無くても、
DiscordChatExporter等でチャンネルをJSON形式にエクスポートしてもらえれば、そのファイルを管理者ページから
アップロードするだけでサイトに反映できます(自動更新はできないため、エクスポートするたびに読み込み直してください)。

1. DiscordChatExporterで対象チャンネル(bot-メッセージ等)を **JSON形式** でエクスポートする
2. 管理者ページ(`admin.html`)の「STEP 5: Discordログの取り込み」からそのJSONファイルをアップロードする
3. Discordのニックネームとサイト上のメンバー名の対応を確認し(自動推測されるが、絵文字や表記ゆれがある場合は
   プルダウンで選び直す)、「この内容でサイトに反映する」を押す
4. ランキングページの新しいサブタブ「💰 CPランキング」に反映される

取り込んだデータは各プレイヤーの `discordCP` / `discordTotalMR` / `discordTier` / `discordColor` /
`discordCurrentMRText` と、日ごとの伸びを記録する `discordDailyReports` に保存されます(現時点でランキング
ページの「💰 CPランキング」タブでのみ表示に使用しています)。
